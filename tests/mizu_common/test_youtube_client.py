...
def test_iter_channel_videos_without_published_after_returns_all(
    mocker: Any, mock_oauth_client: GoogleOAuthClient
) -> None:
    """published_afterを指定しない場合、全件取得されること.

    Arrange:
        異なる公開日時の動画を含むAPIレスポンスをモックする。

    Act:
        published_after=Noneでiter_channel_videos()を呼び出す。
    ...
    # Act
    result = list(client.iter_channel_videos("test_channel_id", published_after=None))
